...
def get_user(user_id: int):
    user = (
        session.query(TestUserTable)
        .filter(TestUserTable.id == user_id)
        .first()
    )
    return user
...
def post_user(user: TestUser):
    db_test_user = TestUser(name=user.name, email=user.email)  # type: ignore
...
def put_users(user: TestUser, user_id: int):
    target_user = (
        session.query(TestUserTable)
        .filter(TestUserTable.id == user_id)  # type: ignore
        .first()
    )
